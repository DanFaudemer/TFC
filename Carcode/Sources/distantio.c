...
void distantio_decode_rx_frame(ByteQueue* rx_queue)
{
	...
	else if(byte == 0x01)
	{
		...
		if(id < Log.current_index)
		{
			if(Log.variables[id].writeable == 1)
			{						
				if(BytesInQueue(rx_queue) >= 4 && Log.variables[id].type == 0x00)
                {	
				...
				else if(BytesInQueue(rx_queue) >= 4 && Log.variables[id].type == 0x06)
				{
                    bytes[0] = ForcedByteDequeue(rx_queue);
                    bytes[1] = ForcedByteDequeue(rx_queue);
                    bytes[2] = ForcedByteDequeue(rx_queue);
                ...
                else if(BytesInQueue(rx_queue) >= 1 && Log.variables[id].type == 0x01)
				{
                    bytes[0] = ForcedByteDequeue(rx_queue);
            
					to_uint8 = (uint8_t*)(&bytes[0]);
					
					ptr = (void *)(Log.variables[id].ptr);
					tmp_uint8 = (uint8_t*)(ptr);
							
					*tmp_uint8 = *to_uint8;
				}
                else if(BytesInQueue(rx_queue) >= 2 && Log.variables[id].type == 0x05)
				{
                    bytes[0] = ForcedByteDequeue(rx_queue);
                    bytes[1] = ForcedByteDequeue(rx_queue);
                    
					to_int16 = (int16_t*)(&bytes[0]);
					
					ptr = (void *)(Log.variables[id].ptr);
					tmp_int16 = (int16_t*)(ptr);
