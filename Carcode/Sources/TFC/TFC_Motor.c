...
#define FTM0_OVERFLOW_FREQUENCY 5000							  // desired freq of PWM
...
void TFC_InitMotorPWM()
{
    ...
    TPM0_MOD = FTM0_CLOCK/(1<<FTM0_CLK_PRESCALE)/FTM0_OVERFLOW_FREQUENCY; // 4800 -> 200us = 5kHz 
    ...
    //Enable the FTM functions on the the port C
...
//	TPM0_C2V = (uint16_t) ((float)TPM0_MOD * (float)((MotorA + 1.0)/2.0));
	TPM0_C2V = TPM0_MOD * (MotorA+1000)/2000;
    TPM0_C3V = TPM0_C2V;
//	TPM0_C0V = (uint16_t) ((float)TPM0_MOD * (float)((MotorB + 1.0)/2.0));
	TPM0_C0V = TPM0_MOD * (MotorB+1000)/2000;
    TPM0_C1V = TPM0_C0V;
