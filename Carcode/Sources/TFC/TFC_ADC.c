...
void InitADC0()
{
    ...
    Master_Adc0_Config.CONFIG2 = MUXSEL_ADCA // select the A side of the ADC channel.
								| ADACKEN_DISABLED
								| ADHSC_HISPEED
								| ADC_CFG2_ADLSTS(ADLSTS_2);//Extra long sample Time (2 extra clocks)
	...
	switch(CurrentADC_State)
	{
		...
		case ADC_STATE_CAPTURE_LINE_SCAN:
					...
					if(CurrentLineScanPixel<128)
					{
						...
						//{
						//	LineScanImage0WorkingBuffer[CurrentLineScanPixel] = ADC0_RA; // récupère valeur
						//	ADC0_SC1A  =  TFC_LINESCAN1_ADC_CHANNEL | ADC_SC1_AIEN_MASK; // select channel
					...
					//	{
						...
						//	CurrentLineScanChannel = 0;
							...
							TAOS_CLK_LOW; /* une periode de clock  : 2MHz */
								for(Junk = 0;Junk<10;Junk++) // initialement : 50
									{
									}
							TAOS_CLK_HIGH;
